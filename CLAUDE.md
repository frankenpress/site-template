...
- `composer.json` — slim deps. `roots/wordpress` (no-content WP core), `roots/wp-config`, `roots/bedrock-autoloader` (loads composer-installed mu-plugins), `roots/bedrock-disallow-indexing`, `vlucas/phpdotenv`, `oscarotero/env`, `eightoeight/mu-plugin`, `wpackagist-theme/twentytwentyfive` as a default theme. **No WooCommerce, no opinionated plugins.**
...
- `Dockerfile` — multi-stage: composer install → `FROM ghcr.io/eightoeight/runtime:php8.3` (overridable for local dev). **Removes the runtime-baked `mu-plugins/fp/`** so the composer-installed canonical copy is the only one that loads.
...
- **`humanmade/s3-uploads` is a transitive dep** of `eightoeight/mu-plugin`. Don't `composer require` it directly — that risks version drift.
